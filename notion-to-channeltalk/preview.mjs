...
async function fetchPage(client, pageId, includeSubPages, depth = 0) {
  ...
  let cursor;
  do {
    const res = await client.blocks.children.list({ block_id: id, start_cursor: cursor, page_size: 100 });
  ...
  if (includeSubPages && depth < 5) {
    for (const b of allBlocks.filter(b => b.type === "child_page")) {
      try { children.push(await fetchPage(client, b.id, includeSubPages, depth + 1)); } catch {}
    }
  }
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .preview-box h3{font-size:14px;font-weight:600;margin-bottom:10px;color:#1d1d1f}
  ...
  .empty{text-align:center;color:#8e8e93;font-size:14px;padding:32px}
</style>
</head>
<body>
<div class="wrap">
...
<script>
let previewedPages = null;

function v(id) { return document.getElementById(id).value.trim(); }

// Channel Talk 자격증명 변경 시 자동 검증
let validateTimer;
...
async function validateCT() {
  ...
  if (res.ok) {
    el.innerHTML = '<div class="badge ok">✓ ' + res.space.name + '</div>';
  } else {
    el.innerHTML = '<div class="badge err">✗ ' + res.error + '</div>';
  }
}
...
function getParams() {
  return {
    notionToken: v('notionToken'),
    notionUrl: v('notionUrl'),
    accessKey: v('accessKey'),
    accessSecret: v('accessSecret'),
    includeSubPages: document.getElementById('includeSubPages').checked,
  };
}
...
async function doPreview() {
  ...
  if (!res.ok) {
    document.getElementById('previewArea').innerHTML =
      '<div class="card" style="color:#c0392b">✗ ' + res.error + '</div>';
    return;
  }

  previewedPages = res.pages;
  document.getElementById('btnSync').disabled = false;

  const html = '<div class="card"><div class="section-title">미리보기 — ' + res.pages.length + '개 페이지</div>' +
    res.pages.map((p, i) =>
      '<div class="page-item">' +
      '<div class="page-header" onclick="togglePage(' + i + ')">' +
      '<div><div class="page-title">' + esc(p.title) + '</div>' +
      '<div class="page-meta">' + (p.size > 1024 ? (p.size/1024).toFixed(1)+'KB' : p.size+'B') + '</div></div>' +
      '<span id="arrow' + i + '">▶</span>' +
      '</div>' +
      '<div class="page-body" id="body' + i + '" style="display:none">' + (p.preview || '<em>내용 없음</em>') + (p.size > 500 ? '…' : '') + '</div>' +
      '</div>'
    ).join('') + '</div>';
...
async function doSync() {
  ...
  const html = '<div class="card">' +
    '<div class="section-title">결과 — 성공 ' + success + ' / 실패 ' + fail + '</div>' +
    res.results.map(r =>
